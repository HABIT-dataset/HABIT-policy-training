...
### Fine-tune HABIT (Bimanual Franka)

We provide a ready-to-use fine-tuning recipe for the HABIT setup, a bimanual Franka configuration with a 14D state / 14D delta-action space and three camera views (`front_view`, `left_wrist_view`, `right_wrist_view`).

#### Dataset Format

Convert your demonstrations into the **GR00T-flavored LeRobot v2 format** (see [`getting_started/data_preparation.md`](getting_started/data_preparation.md)). The dataset must expose the modality keys defined in [`examples/HABIT/habit_config.py`](examples/HABIT/habit_config.py):

- **State (14D)**: `cartesian_position_xyz_left`, `cartesian_position_rot_left`, `gripper_position_left`, `cartesian_position_xyz_right`, `cartesian_position_rot_right`, `gripper_position_right`
- **Action (14D, 16-step horizon)**: `delta_action_eef_left`, `delta_action_gripper_left`, `delta_action_eef_right`, `delta_action_gripper_right`
- **Video**: `right_wrist_view`, `left_wrist_view`, `front_view`
- **Language**: `annotation.human.action.task_description`

#### Run Fine-tuning

Edit the dataset path at the top of the script, then run.

**Single GPU:**
```bash
bash examples/HABIT/finetune_habit_1gpu.sh
```

**Single node (8 GPUs via `torchrun`):**
```bash
bash examples/HABIT/finetune_habit_1node.sh
```

Checkpoints are written to `./outputs/habit_finetune` by default.
